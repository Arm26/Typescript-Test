function getHandScore(input: string): number {
  const suits: { [key: string]: number } = { 'H': 0, 'C': 0, 'D': 0, 'S': 0 }; // จำนวนไพ่ในแต่ละสัญลักษณ์
  const ranks: { [key: string]: number } = {}; // จำนวนไพ่ที่มีค่าเท่ากัน
  let score: number = 0; // แต้มที่ได้

  const hand: string[] = input.split(' '); // แยกไพ่ในมือเป็นอาร์เรย์

  for (const card of hand) {
    const suit: string = card.slice(0, 1); // สัญลักษณ์ของไพ่
    const rank: string = card.slice(1); // เลขหรือตัวอักษรของไพ่
  ...
  const maxSuitValue: number = Math.max(...Object.values(suits));
