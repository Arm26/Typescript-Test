...
function getQuestionPart(phrases: string[]): string[] {
  const questionPart: string[] = []; // สร้างตัวแปร questionPart เพื่อเก็บค่า String

  //กำหนดค่า i ตามจำนวนข้อความ
  for (let i = 0; i < phrases.length; i++) {
    const phrase = phrases[i]; // ดึงคำศัพท์ออกมาจากอาร์เรย์ phrases เก็บใน phrase
    const commonLetters: string[] = []; // สร้างตัวแปร commonLetters เพื่อเก็บตัวอักษรที่เหมือนกัน
